...
document.addEventListener('DOMContentLoaded', function() {
  ...
  window.addEventListener('scroll', () => {
    ...
    if (scrollY >= 1000) {
      topPageBtn.classList.add('is-show');
    } else {
      topPageBtn.classList.remove('is-show');
    }
  });


  // ページ上部に戻るボタンの挙動
  topPageBtn.addEventListener('click', (event) => {
  ...
  // グロナビの「CATEGORY」をホバーしたら吹き出しが出る処理

  // ヘッダーの「CATEGORY」の要素
  const headerNavCategory = document.querySelectorAll('.l-header-nav__link')[0];
  // ヘッダーの吹き出しの要素
  const headerCategoryTxtboxPc = document.querySelector('.l-header-category-txtbox--pc');

  // ヘッダーの「CATEGORY」の要素をホバーすると吹き出しが出る(「Search」の吹き出しは消える)
  headerNavCategory.addEventListener('mouseover', () => {
    headerCategoryTxtboxPc.classList.add('is-show');
    headerSearchTxtboxPc.classList.remove('is-show');
  });

  // ヘッダーの「CATEGORY」の要素以外の場所をクリックすると吹き出しが消える
  document.addEventListener('click', (event) => {
    event.preventDefault();
    if(!event.target.closest('.l-header-category-txtbox--pc')) {
      headerCategoryTxtboxPc.classList.remove('is-show');
    } else {
    }
  });

  // ヘッダーの「CATEGORY」の閉じるボタンをクリックすると吹き出しが消える
  const headerCategoryCloseBtn = document.querySelector('.l-header-category-txtbox--pc .c-txtbox-closeBtn');
  headerCategoryCloseBtn.addEventListener('click', () => {
    headerCategoryTxtboxPc.classList.remove('is-show');
  });

  // ヘッダーの「Search」をフォーカスすると、吹き出しが出る(カテゴリーの吹き出しは消える)
  const headerSearchTxtboxPc = document.querySelector('.l-header-search-txtbox--pc');
  const headerSearchPcInput = document.querySelector('.c-header-search--pc input');
  headerSearchPcInput.addEventListener('focus', () => {
    headerSearchTxtboxPc.classList.add('is-show');
    headerCategoryTxtboxPc.classList.remove('is-show');
  });

  // ヘッダーの「Search」のフォーカスが外れると、吹き出しが消える
  headerSearchPcInput.addEventListener('blur', () => {
    headerSearchTxtboxPc.classList.remove('is-show');
  });

  // PCのヘッダーの吹き出しのリンクのスクロール
  const headerCategoryTxtboxPcLinks = [...document.querySelectorAll('.l-header-category-txtbox--pc a , .l-slide-nav__link')];
  let headerCategoryTxtboxPcLinksHref = headerCategoryTxtboxPcLinks.map((elem) => {
    return elem.getAttribute('href');
  });

  headerCategoryTxtboxPcLinks.forEach((elem , index) => {
    elem.addEventListener('click', () => {
      let scrollPosHref = headerCategoryTxtboxPcLinksHref[index];
      let scrollPosHrefId = document.getElementById(scrollPosHref.replace('#' , ''));
      scrollPosHrefId.scrollIntoView({  
        behavior: 'smooth'  
      });
    });
  })
...
const slideParts = document.querySelectorAll('.c-hamburger span , .l-header__inner , .l-slide-nav , .c-modal');
hamburger.addEventListener('click', () => {
  slideParts.forEach((elem) => {
    elem.classList.toggle('is-slide');
  })
});
...
modal.addEventListener('click', () => {
  slideParts.forEach((elem) => {
    elem.classList.remove('is-slide');
  })
});
